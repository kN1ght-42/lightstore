<!DOCTYPE html>
<html lang="ru">

<head>
	<title>Сравнение</title>
	<?php include './assets/includes/head.php'; ?>
</head>

<body>
	<?php include './assets/includes/feedback-modal-window.php'; ?>
	<?php include './assets/includes/header.php'; ?>
	<!-- Page-headline -->
	<section class="page-headline">
		<div class="container">
			<div class="page-headline__inner page-headline__inner--cart">
				<div class="page-headline__left">
					<button class="back__link">
						<svg class="back__link__svg">
							<use xlink:href="#arrow" />
						</svg>
					</button>
					<div class="bread__crumbs">
						<a href="#" class="bread__crumbs__link">Главная</a>
						<span>/</span>
						<a href="#" class="bread__crumbs__link bread__crumbs__link--current">Избранное</a>
					</div>
				</div>
			</div>
		</div>
	</section>
	<!-- /Page-headline -->
	<!-- Compare -->
	<section class="compare-block">
		<div class="container">
			<div class="compare-block__inner">
				<div class="compare-block__inner__top">
					<div class="title">
						Сравнение товаров
						<span>6 товаров</span>
					</div>
					<button class="clear__favorite">
						Очистить <span>весь список</span>
						<span class="count">6</span>
					</button>
				</div>
				<div class="compare__goods store__goods">
					<div class="container">
						<div class="store__goods__inner">
							<div class="goods__tags__wrapper">
								<div class="store__goods__tags">
									<div class="store__goods__tags__inner">
										<div class="store__good__tag">
											<input type="radio" name="goodSwitcher" id="goodItem0" checked>
											<label for="goodItem0">Люстры<span>5</span></label>
										</div>
										<div class="store__good__tag">
											<input type="radio" name="goodSwitcher" id="goodItem1">
											<label for="goodItem1">Светильники<span>5</span></label>

										</div>
										<div class="store__good__tag">
											<input type="radio" name="goodSwitcher" id="goodItem2">
											<label for="goodItem2">Бра<span>1</span></label>
										</div>
									</div>
									<div class="store__goods__tags__buttons">
										<button class="arrow__button prev arrow__button--small">
											<svg class="arrow__button__svg">
												<use xlink:href="#arrow" />
											</svg>
										</button>
										<button class="arrow__button next arrow__button--small">
											<svg class="arrow__button__svg">
												<use xlink:href="#arrow" />
											</svg>
										</button>
									</div>
								</div>
							</div>
							<div class="store__goods__tabs">
								<div class="store__goods__wrapper goodItem0 store__goods__wrapper--active">
									<div class="store__goods__slider">
										<div class="store__goods__products">
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />
													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
										</div>
										<div class="store__goods__products__buttons">
											<button class="arrow__button prev arrow__button--small">
												<svg class="arrow__button__svg">
													<use xlink:href="#arrow" />
												</svg>
											</button>
											<button class="arrow__button next arrow__button--small">
												<svg class="arrow__button__svg">
													<use xlink:href="#arrow" />
												</svg>
											</button>
										</div>
									</div>
								</div>
								<div class="store__goods__wrapper goodItem1">

									<div class="store__goods__slider">
										<div class="store__goods__products">
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />
													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
										</div>
										<div class="store__goods__products__buttons">
											<button class="arrow__button prev arrow__button--small">
												<svg class="arrow__button__svg">
													<use xlink:href="#arrow" />
												</svg>
											</button>
											<button class="arrow__button next arrow__button--small">
												<svg class="arrow__button__svg">
													<use xlink:href="#arrow" />
												</svg>
											</button>
										</div>
									</div>
								</div>
								<div class="store__goods__wrapper goodItem2">
									<div class="store__goods__slider">
										<div class="store__goods__products">
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />
													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
											<div class="good__item">
												<div class="good__item__top">
													<div class="good__item__tags">
														<div class="good__item__tag">
															Новинка
														</div>
													</div>
													<div class="good__item__reactions">
														<button class="compare">
															<svg>
																<use xlink:href="#compare" />
															</svg>
														</button>
														<button class="like active">
															<svg>
																<use xlink:href="#favourite" />
															</svg>
														</button>
													</div>
												</div>
												<div class="store__good__images" style="text-align: center">
													<img src="./assets/images/goodItems/goodItem1.jpeg" alt="" class="good__item__image good__item__image--active" id="" />
													<img src="./assets/images/goodItems/goodItem2.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem3.jpeg" alt="" class="good__item__image" id="" />
													<img src="./assets/images/goodItems/goodItem4.jpeg" alt="" class="good__item__image" id="" />

													<div class="good__item__image-switcher"></div>
												</div>
												<div class="good__item__bonuses">
													<div class="good__item__bonus sale">
														Скидка 20%
													</div>
												</div>
												<div class="good__item__image__buttons"></div>
												<div class="good__item__info">
													<div class="articule">
														Артикул: 351018510
													</div>
													<div class="title">
														Настенный светильник (бра) Freya
														FR5133WL-01CH
													</div>
													<div class="bottom__info bottom__info--secondary">
														<div class="prices">
															<div class="current__price">
																85 672 ₽
															</div>
															<div class="old__price">
																95 672 ₽
															</div>
														</div>
														<div class="buttons">
															<button class="good__item__button buy">
																Купить
															</button>
															<button class="good__delete">
																<svg>
																	<use xlink:href="#delete" />
																</svg>
															</button>
														</div>
													</div>

												</div>
											</div>
										</div>
										<div class="store__goods__products__buttons">
											<button class="arrow__button prev arrow__button--small">
												<svg class="arrow__button__svg">
													<use xlink:href="#arrow" />
												</svg>
											</button>
											<button class="arrow__button next arrow__button--small">
												<svg class="arrow__button__svg">
													<use xlink:href="#arrow" />
												</svg>
											</button>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>
	<!-- /Compare -->
	<!-- Good-info -->
	<section class="good-info">
		<div class="container">
			<div class="good-info__inner">
				<div class="good-info__inner__top">
					<div class="good-info__top__item">
						<label class="yellow__checkbox">
							<input type="radio" name="compareRadio" id="allСharacteristics" checked />
							<span class="checkmark"></span>
						</label>
						<label class="title" for="allСharacteristics">Все характеристики</label>
					</div>
					<div class="good-info__top__item">
						<label class="yellow__checkbox" style="">
							<input type="radio" name="compareRadio" id="differentСharacteristics" />
							<span class="checkmark"></span>
						</label>
						<label class="title" for="differentСharacteristics">Показать только различия</label>
					</div>
				</div>
				<div class="good-info__inner__main">
					<div class="good-info__section">
						<div class="good-info__section__title">Размеры изделия</div>
						<div class="good-info__section__items">
							<div class="good-info__section__item">
								<div class="title">Высота (мм)</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="1">
										<span>1</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="2">
										<span>2</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="3">
										<span>3</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="4">
										<span>4</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="5">
										<span>5</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="6">
										<span>6</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Диаметр (мм)</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>1</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>2</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>3</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>4</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>5</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>6</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Ширина (мм)</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>1</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>2</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>3</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>4</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>5</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>6</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Диаметр абажура-плафона (мм)</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>1</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>2</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>3</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>4</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>5</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>6</span>
									</span>
								</div>
							</div>
						</div>
					</div>
					<div class="good-info__section">
						<div class="good-info__section__title">Размеры упаковки</div>
						<div class="good-info__section__items">
							<div class="good-info__section__item">
								<div class="title">Высота коробки (см)</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>1</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>2</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>3</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>4</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>5</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>6</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Длина коробки (см)</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>1</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>2</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>3</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>4</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>5</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>6</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Ширина коробки (см)</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>1</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>2</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>3</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>4</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>5</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>6</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Вес в упаковке (кг)</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>1</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>2</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>3</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>4</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>5</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>6</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Объем упаковки</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>1</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>2</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>3</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>4</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>5</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="360">
										<span>6</span>
									</span>
								</div>
							</div>
						</div>
					</div>
					<div class="good-info__section">
						<div class="good-info__section__title">Дополнительная информация</div>
						<div class="good-info__section__items">
							<div class="good-info__section__item">
								<div class="title">Степень защиты (IP)</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="IP20">
										<span>IP20</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="IP20">
										<span>IP20</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="IP20">
										<span>IP20</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="IP20">
										<span>IP20</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="IP20">
										<span>IP20</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="IP20">
										<span>IP20</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Способ крепления</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="Планка">
										<span>Планка</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Планка">
										<span>Планка</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Планка">
										<span>Планка</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Планка">
										<span>Планка</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Планка">
										<span>Планка</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Планка">
										<span>Планка</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Вес (Кг)</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="1.8">
										<span>1.8</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="1.8">
										<span>1.8</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="1.8">
										<span>1.8</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="1.8">
										<span>1.8</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="1.8">
										<span>1.8</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="1.8">
										<span>1.8</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Штрихкод</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="4251110055428">
										<span>4251110055428</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="4251110055428">
										<span>4251110055428</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="4251110055428">
										<span>4251110055428</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="4251110055428">
										<span>4251110055428</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="4251110055428">
										<span>4251110055428</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="4251110055428">
										<span>4251110055428</span>
									</span>
								</div>
							</div>
						</div>
					</div>
					<div class="good-info__section">
						<div class="good-info__section__title">Внешний вид</div>
						<div class="good-info__section__items">
							<div class="good-info__section__item">
								<div class="title">Преобладающий цвет</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Цвет основания</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Цвет плафона</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Белый">
										<span>Белый</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Материал основания</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="Металл, Дерево">
										<span>Металл, Дерево</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Металл, Дерево">
										<span>Металл, Дерево</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Металл, Дерево">
										<span>Металл, Дерево</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Металл, Дерево">
										<span>Металл, Дерево</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Металл, Дерево">
										<span>Металл, Дерево</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Металл, Дерево">
										<span>Металл, Дерево</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Материал плафона</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="Стекло">
										<span>Стекло</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Стекло">
										<span>Стекло</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Стекло">
										<span>Стекло</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Стекло">
										<span>Стекло</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Стекло">
										<span>Стекло</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Стекло">
										<span>Стекло</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Виды материалов</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="Дерево">
										<span>Дерево</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Дерево">
										<span>Дерево</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Дерево">
										<span>Дерево</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Дерево">
										<span>Дерево</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Дерево">
										<span>Дерево</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Дерево">
										<span>Дерево</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Форма</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="Шары">
										<span>Шары</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Шары">
										<span>Шары</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Шары">
										<span>Шары</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Шары">
										<span>Шары</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Шары">
										<span>Шары</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Шары">
										<span>Шары</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Декор</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="Ретро, Под старину">
										<span>Ретро, Под старину</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Ретро, Под старину">
										<span>Ретро, Под старину</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Ретро, Под старину">
										<span>Ретро, Под старину</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Ретро, Под старину">
										<span>Ретро, Под старину</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Ретро, Под старину">
										<span>Ретро, Под старину</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Ретро, Под старину">
										<span>Ретро, Под старину</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Форма плафона</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="Шар">
										<span>Шар</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Шар">
										<span>Шар</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Шар">
										<span>Шар</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Шар">
										<span>Шар</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Шар">
										<span>Шар</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="Шар">
										<span>Шар</span>
									</span>
								</div>
							</div>
						</div>
					</div>
					<div class="good-info__section">
						<div class="good-info__section__title">Электрика</div>
						<div class="good-info__section__items">
							<div class="good-info__section__item">
								<div class="title">Количество ламп (Шт)</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="6">
										<span>6</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="6">
										<span>6</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="6">
										<span>6</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="6">
										<span>6</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="6">
										<span>6</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="6">
										<span>6</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Цоколь</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="E27">
										<span>E27</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="E27">
										<span>E27</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="E27">
										<span>E27</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="E27">
										<span>E27</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="E27">
										<span>E27</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="E27">
										<span>E27</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Мощность лампы (Вт)</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="60">
										<span>60</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="60">
										<span>60</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="60">
										<span>60</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="60">
										<span>60</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="60">
										<span>60</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="60">
										<span>60</span>
									</span>
								</div>
							</div>
							<div class="good-info__section__item">
								<div class="title">Напряжение (В)</div>
								<div class="values">
									<span class="value__item">
										<input type="hidden" name="" value="220">
										<span>220</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="220">
										<span>220</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="220">
										<span>220</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="220">
										<span>220</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="220">
										<span>220</span>
									</span>
									<span class="value__item">
										<input type="hidden" name="" value="220">
										<span>220</span>
									</span>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>
	<!-- /Good-info -->

	<?php include './assets/includes/defaults/our-advantages/our-advantages.php'; ?>
	<?php include './assets/includes/footer.php'; ?>
	<?php include './assets/includes/copyright.php'; ?>
	<?php include './assets/includes/scripts.php'; ?>
</body>

</html>